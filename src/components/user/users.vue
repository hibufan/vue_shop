<template>
  <div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="setRoleDivialogVisible = false">取 消</el-button>
...
export default {
  ...
  methods: {
    ...
    // 修改用户信息并提交
    editUserInfo() {
      this.$refs.editFormRef.validate(async (valia) => {
        if (!valia) return;
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put(
    ...
    async setRole(userinfo) {
      ...
      this.setRoleDivialogVisible = true;
    },
    // 点击按钮，分配角色
    async saveRoleInfo() {
